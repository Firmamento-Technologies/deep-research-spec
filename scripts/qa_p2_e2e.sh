...
check_frontend_toolchain() {
  ...
  if [[ ! -f "frontend/node_modules/.bin/vite" ]]; then
    echo "vite binary shim missing at frontend/node_modules/.bin/vite"
    return 1
  fi
...
run_check fail "Frontend runtime bootstrap" bash -lc "BOOTSTRAP_FRONTEND_DEPS=${QA_BOOTSTRAP_FRONTEND_DEPS} bash scripts/ensure_frontend_runtime.sh"
