...
- **LeanAide** (`siddhartha-gadgil/LeanAide`) — autoformalization: traduce prove in linguaggio naturale in Lean 42_10]
- **Delta Prover** (arXiv 2503.18102) — agent framework che orchestra un LLM general-purpose con Lean 4 direttamente2_11]
- **Lean LSP MCP Server** — permette all'AI CLI di interrogare Lean per type checking e goal state in tempo reale2_1]
...
1. **Clona Alethfeld** — ha già il core loop Prover/Verifier e l'integrazione Lean 42_1]
...
2_17]2_18]2_19]2_20]2_21]2_22]2_23]2_24]2_25]2_26]2_27]2_28]2_29]2_30]
...
3_10]3_11]3_12]3_13]3_14]3_15]3_9]
...
3. Il contatore iterazioni è nello State — impedisce loop infiniti4_5]
...
- **`madaan/self-refine`** (MIT) — implementazione di riferimento del loop feedback, pronto all'uso4_3]
- **`junfanz1/LangGraph-Reflection-Researcher`** — già combina LangGraph + Reflexion + web search per research iterativo4_7]
- **`langchain-ai/open_deep_research`** — base per il layer di ricerca, da connettere al loop4_9]
- **LangGraph `v1.0`** — il runtime per i cicli condizionali con State persistente4_10]
...
- **DeepSeek-R1** → eccelle in ragionamento logico e coerenza strutturale5_2]
- **Qwen3-14B** → forte su accuratezza fattuale e citazioni5_5]
- **Mistral Large** → bilanciato, meno "sycophantic" degli altri due5_6]
- **(opzionale) Llama-3.3-70B** → quarto voto di spareggio, addestrato Meta con dati differenti5_6]
...
5_10]5_11]5_12]5_13]5_14]5_15]5_16]5_9]
...
9_10]9_11]9_12]9_13]9_14]9_15]9_7]9_8]9_9]
...
10_10]10_11]10_12]10_13]10_14]10_15]10_16]10_9]
...
11_10]11_11]11_12]11_13]11_14]11_15]11_6]11_7]11_8]11_9]
...
12_10]12_11]12_12]12_13]12_14]12_15]12_6]12_7]12_8]12_9]
...
13_10]13_11]13_12]13_13]13_14]13_15]
...
14_10]14_11]14_12]14_13]14_14]14_15]14_6]14_7]14_8]14_9]
...
async def _call_with_retry(model, system_prompt, user_prompt, temperature,
    ...
    return {
        "content":    response.choices24_0].message.content,
        "model_used": model,
        "tokens_in":  tokens_in,
...
5. Richiedere che la fusione sia invisibile — il lettore non deve percepire giunture tra stili diversi27_2]27_3]27_1]
...
expected_output:
  ...
  feedback_items46_0].category: "factual"
  feedback_items46_0].priority: "CRITICAL"
  feedback_items46_0].original_must_contain: "45.000"
...
Ogni slot agente ha un **context budget** fisso, calcolato come percentuale della MECW del modello assegnato (non della MCW nominale).3]4]
...
Questi fattori sono conservativi ma motivati: la MECW è task-specific e i task di ragionamento complesso degradano più velocemente dei task estrattivi.1]
...
Se una delle categorie supera il suo budget, il Context Compressor interviene prima della chiamata — non dopo.5]

***

## Context Compressor: strategie per documenti enormi

Per documenti >15.000 parole o con molte sezioni, il contesto delle sezioni approvate cresce fino a saturare il budget. Il Context Compressor applica strategie diverse in cascata:6]4]
...
Il Context Compressor identifica quali porzioni delle sezioni precedenti sono rilevanti per la sezione corrente (basandosi sul topic dello scope) e include solo quelle. RAG sulla memoria interna del documento.5]
...
Ogni provider LLM ha rate limit diversi — Requests Per Minute (RPM) e Tokens Per Minute (TPM). Il DRS può avere decine di run parallele che fanno centinaia di chiamate al minuto. Gestirli male significa 429 a cascata.7]
...
Prima di ogni chiamata LLM, il sistema verifica disponibilità nel bucket. Se il bucket è pieno, la chiamata viene messa in coda con priorità: le chiamate della giuria hanno priorità più alta di quelle del Researcher (il blocco è più costoso).7]
...
KEDA (Kubernetes Event-Driven Autoscaling) scala i worker Celery in base alla lunghezza della queue Redis. La formula di scaling è:8]
...
**Stateless workers:** ogni Celery worker è completamente stateless — lo State del run è in PostgreSQL/Redis, non in memoria del worker. Un worker può morire a metà run e il job viene riassegnato automaticamente a un altro worker che riprende dall'ultimo checkpoint.8]
...
La ricerca arXiv (novembre 2025) documenta il problema fondamentale del testing per sistemi LLM: i test puramente funzionali non colgono i fallimenti di ragionamento, ma il "LLM-as-judge" ha bias intrinseci. La soluzione adottata dalla letteratura è un **framework ibrido** che combina i due approcci.9]10]
...
**Target:** 100% di copertura su tutti i path deterministici. Questi test devono essere veloci (<5s totali) e girare ad ogni commit.11]
...
**Frequenza:** ad ogni PR che tocca il grafo LangGraph o i prompt. Non ad ogni commit (troppo costoso).11]
...
Non si confronta testo con testo (i documenti saranno diversi per definizione). Si confrontano su 5 dimensioni:12]9]
...
**Soglia di pass:** DQS ≥ 0.75 su tutti e 10 i documenti del Golden Set. Se un documento scende sotto 0.70, il deploy viene bloccato.9]

***

## Layer 5 — Test di regressione automatici

Eseguiti automaticamente ogni domenica notte e dopo ogni deploy in produzione. Usano un subset di 3 documenti del Golden Set (i più rappresentativi) per verificare che nessuna regressione sia stata introdotta.13]
...
Una volta al mese, in ambiente staging, si eseguono test di chaos per verificare che la resilienza funzioni davvero:8]
