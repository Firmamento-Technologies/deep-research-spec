Looking at the issue, I need to fix §16.3 to clarify that `draft_embedding` is computed by MetricsCollector, and I need to add the embedding field to §5.8 MetricsCollector's OUTPUT schema. However, §5.8 is not in this file (it's in a different spec file). Since I can only fix the current file (16_writer_memory.md), I need to:

1. Add a clear note in §16.3 specifying that MetricsCollector must be extended to produce `draft_embedding`
2. Add a CONSTRAINT block clarifying the model and storage behavior
3. Reference §5.8 explicitly so the fix is traceable
