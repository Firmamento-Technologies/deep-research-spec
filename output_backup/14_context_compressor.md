...
CONSTRAINTS:
  ...
  ALWAYS run after section checkpoint (§21.2), never before
...
```
[section_checkpoint node] → [context_compressor node] → [coherence_guard node] → [route_next_section]
```

- **Trigger**: `section_checkpoint` emits approval signal.
...
```python
g.add_edge("section_checkpoint", "context_compressor")   # runs after every approval
