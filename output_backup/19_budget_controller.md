...
PRODUCES: [budget.estimated_total_usd, budget.regime, budget.blocked,
           budget.css_threshold, budget.max_iterations, budget.jury_size] -> DocumentState
```

## 19.2 Regime Table

| Regime | budget_per_word ($/word) | css_threshold | max_iter | jury_size | MoW | models per slot |
|---|---|---|---|---|---|---|
...
| `spent ≥ 0.90 × max` | `css_threshold = 0.65`; `max_iterations = 1`; `jury_size = 1`; disable MoW; emit BUDGET_ALERT_90 |
| `spent ≥ 1.00 × max` | Hard stop: save LangGraph checkpoint; return partial doc from Store (§21.1); emit BUDGET_HARD_STOP |
| Single section cost > $15.00 | Pause run; emit `BUDGET_SECTION_ANOMALY`; await human via §24.3 `/approve` |
...
|---|---|---|---|---|---|
| `css_threshold` | float | 0.65 | 0.70 | 0.78 | init + 90% alarm |
...
async def run_jury(state: DocumentState) -> dict:
    ...
    css_threshold: float = state["budget"]["css_threshold"]
    ...
    if state["current_iteration"] >= max_iter:
        ...
        # reads this in route_after_aggregator() and routes to 'approved'
        import logging
        logging.warning(
...
CONSTRAINTS:
  ...
  NEVER reduce css_threshold below 0.45
