...
1. `google/gemini-2.5-pro` output price: §28.4 says `5.00`, §33.6 says `10.0` — need canonical source
2. `qwen/qwq-32b` pricing: §28.4 says `{"in": 0.15, "out": 0.60}`, §33.6 says `{"in": 0.12, "out": 0.18}` — need canonical source

The fix requires: (1) removing the `MODEL_PRICING` dict from §33.6 and replacing it with an import reference to a canonical file, and (2) documenting the verification. Since §28.4 is the canonical pricing section referenced throughout the spec, §33.6 should defer to it.
