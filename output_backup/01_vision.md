...
1. **CSS thresholds in §1.4.1**: Change `≥0.50 Balanced | ≥0.45 Premium` to `≥0.70 Balanced | ≥0.78 Premium` (matching §9.3 as source of truth)
2. **`target_words` max in §1.4**: Already correct at `le=50_000`. No change needed here — the issues say §3.3 and §29.6 need fixing, but those sections don't exist in this file. The validation table in §1.4 already shows `50_000`.

Let me re-read carefully... The file contains §1.4 `MandatoryParams` with `le=50_000` ✓ and the validation table with `50_000` ✓. The issues with §3.3 and §29.6 are in other files, not this one.

The only fix needed in this file is the CSS threshold comment in §1.4.1.
