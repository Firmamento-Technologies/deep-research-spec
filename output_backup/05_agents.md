# 05_agents.md — Agent Specifications §5.1–5.21
...
CONSUMES: [sanitized_sources, current_section_idx] from DocumentState
PRODUCES: [compressed_corpus] -> DocumentState (ephemeral per section)
```
...
MoW activation: see §7.1. When MoW active, three Writer instances run in parallel with angles from §7.2; results feed §5.13.
...
Note: the node formerly named `span_editor_integrate` is consolidated into `span_editor`. The
`diff_merger` node applies `apply_edits()` deterministically; `style_linter` re-runs to catch any
L1/L2 violations introduced by replacement text before the draft re-enters the jury path.
...
CONSTRAINTS:
  ...
  ALWAYS run exactly once per section on first iteration (gated by current_iteration == 1 in MoW
    activation check at §7.1; the Fusor constraint here is a local invariant confirming this)
...
PRODUCES: [current_draft (fused)] -> DocumentState
```

MoW jury flow: on the first iteration of a section, the graph routes through `jury_multi_draft`
(see §4.5 node definition) which evaluates the three W-A/W-B/W-C drafts in parallel, produces
per-draft CSS scores and `best_elements`, then transitions unconditionally to `fusor`. The fused
draft then enters the standard `jury` node for the official CSS evaluation whose result is recorded
in `css_history`. On all subsequent iterations (current_iteration > 1), MoW is not activated and
the graph goes directly to the standard `writer` → `jury` path; Fusor is never re-invoked.
...
PRODUCES: [reflector_output] -> DocumentState; routes to SpanEditor (SURGICAL) or Writer (PARTIAL) or await_human (FULL)
```
...
CONSTRAINTS:
  MUST apply position-based strategy:
  ...
  ALWAYS invoke after each section approval, before next section starts
ERROR_HANDLING:
...
CONSTRAINTS:
  ...
  ALWAYS run before section_checkpoint node
ERROR_HANDLING:
...
MODEL: none — deterministic / TEMP: n/a / MAX_TOKENS: n/a
INPUT:
  current_draft: str                  # content approved by jury (CSS_content ≥0.65, CSS_style ≥0.80)
...
CONSTRAINTS:
  ...
  ALWAYS run after CoherenceGuard (§5.17) has returned no HARD conflicts
  ALWAYS run before ContextCompressor (§5.16) is invoked for the next section
ERROR_HANDLING:
...
```python
# After CoherenceGuard resolves without HARD conflict:
g.add_edge('section_checkpoint', 'context_compressor')

# After ContextCompressor produces compressed_context:
g.add_conditional_edges('context_compressor', route_next_section, {
    'next_section': 'researcher',   # more sections remain
    'all_done':     'post_qa'       # all sections approved → Phase C
})
```

The `section_checkpoint` node is the single authoritative point where:
1. A section transitions from `in_progress` to `approved` in PostgreSQL.
2. `current_section_idx` is incremented, gating all subsequent section routing.
3. `writer_memory` is refreshed so the next Writer call inherits accumulated patterns.
4. The SSE dashboard receives the `SECTION_APPROVED` notification.

---

## §4.5 Addendum — jury_multi_draft Node

The following node and edges are added to the LangGraph definition (§4.5) to make the
MoW Phase 1 evaluation explicit and distinguish it from the standard jury path:

```python
# Additional node — MoW Phase 1 jury evaluation
g.add_node('jury_multi_draft', jury_multi_draft.run)
# jury_multi_draft evaluates W-A, W-B, W-C drafts in parallel:
#   - produces per-draft CSS scores (css_individual) and best_elements lists
#   - does NOT record any CSS value into css_history (only the post-Fusor jury does)
#   - transitions unconditionally to fusor regardless of per-draft CSS outcomes
#   - VETO on a draft excludes it as Fusor base; if veto_category==fabricated_source
#     the draft's best_elements are also discarded entirely

g.add_conditional_edges('writer', route_after_writer, {
    'mow_active':    'jury_multi_draft',   # current_iteration == 1 AND MoW enabled
    'standard':      'metrics_collector'   # iteration > 1 OR MoW disabled
})

g.add_edge('jury_multi_draft', 'fusor')
g.add_edge('fusor', 'metrics_collector')   # fused draft enters standard jury path
# metrics_collector → jury → aggregator (standard path, CSS recorded in css_history)

# Surgical edit path (replaces former span_editor_integrate node):
g.add_edge('span_editor', 'diff_merger')
g.add_edge('diff_merger', 'style_linter')  # re-lint after surgical edits
# style_linter → [linter_pass=True] → jury  |  [linter_pass=False] → style_fixer → jury
```

Node `span_editor_integrate` is removed from the NODES list; its function is fully covered by
`span_editor` + `diff_merger`. No other node references `span_editor_integrate`.
...
RouteKey = Literal[
    "approved",          # CSS_content ≥0.65 AND CSS_style ≥0.80 → context_compressor
    "fail_style",        # CSS_content pass, CSS_style <0.80 → style_fixer
    "fail_reflector",    # CSS_content <0.65, scope=PARTIAL → reflector → writer
    "surgical",          # scope=SURGICAL → span_editor → diff_merger → style_linter
    "fail_full",         # scope=FULL → await_human
    "panel",             # CSS_content <0.50 → panel_discussion
    "veto",              # any L1 veto → reflector
    "missing_evidence",  # judge_f.missing_evidence nonempty → researcher_targeted
