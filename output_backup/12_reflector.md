...
| `FULL` | ANY of: dominant `category == "logical_contradiction"` spanning ≥3 paragraphs, ≥1 `severity == "CRITICAL"` with `category == "structural_error"`, Coherence Guard flagged HARD conflict in current section (§15.1) | → human escalation (§13.4) |
...
def route_after_reflector(state: DocumentState) -> str:
    ...
    iteration = state["current_section"]["iteration"]
