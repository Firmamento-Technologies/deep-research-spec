Looking at the issues to fix, I need to change `le=100_000` to `le=50_000` in exactly two places:

1. **§3.3 DRSConfig** - The YAML comment for `target_words` in §29.1 User Section shows `range [500, 100000]`
2. **§29.6 UserConfig** - The `target_words` field validator uses `le=100_000`

Let me trace all three locations mentioned in the issue:
- §1.4 MandatoryParams (`le=50_000`) — this is in the source material's `scheletro_completo.md`, not in this file
- §3.3 DRSConfig (`le=100_000`) — this refers to the YAML annotation comment in §29.1
- §29.6 UserConfig (`le=100_000`) — this is in the Pydantic schema

I need to fix both the YAML comment in §29.1 and the Pydantic field in §29.6.
