...
RESPONSIBILITY: Generate section draft from compressed corpus + memory + exemplar.
  ...
  MoW jury evaluation, and invokes Fusor to produce a single fused draft before
  returning. On all other iterations, a single-draft path is used.
...
single fused draft. The graph sees only a single 'writer' node.
...
def route_after_reflector(state: DocumentState) -> Literal[
    ...
    """
    scope = state["reflector_output"]["scope"]
    if scope == "FULL":
        return "await_human"
...
def route_after_oscillation(state: DocumentState) -> Literal[
    ...
    'continue' routes to span_editor (SURGICAL) or writer (PARTIAL) based
    on the reflector scope stored in state.
    """
...
# 'surgical', 'partial', and 'full' are NOT aggregator-level routes — they are
# scope values determined by route_after_reflector() AFTER the aggregator routes
# to 'fail_reflector' or 'veto' and the Reflector runs.
...
# Stage 2: route_after_reflector() reads reflector_output.scope and returns:
ReflectorRouteKey = Literal[
    "oscillation_check",  # scope=SURGICAL or PARTIAL → oscillation_check
                          #   oscillation_check then routes to:
                          #     span_editor (SURGICAL, no oscillation)
                          #     writer      (PARTIAL, no oscillation)
                          #     await_human (oscillation detected)
