...
| `judge_r2` | `openai/o3-mini` | `deepseek/deepseek-r1` | `qwen/qwq-32b` | `adversarial_reasoning` — o3-mini provides mid-cost reasoning with distinct RLHF pipeline from DeepSeek R1 |
...
**Design note — Reasoning jury slot assignments and per-slot fallback chains:**

The Primary Model column above defines the **decorrelated selection** for each judge slot, ensuring epistemic independence across R1/R2/R3. The `§8.5 JURY_TIERS` YAML block defines the **per-slot fallback sequence** invoked when the primary model for that specific slot is unavailable or budget triggers a downgrade. These are complementary, not contradictory:

- `judge_r1` primary = `deepseek/deepseek-r1`; if R1 primary fails → fallback sequence: [`openai/o3-mini`, `qwen/qwq-32b`]
- `judge_r2` primary = `openai/o3-mini`; if R2 primary fails → fallback sequence: [`deepseek/deepseek-r1`, `qwen/qwq-32b`]
- `judge_r3` primary = `qwen/qwq-32b`; if R3 primary fails → fallback sequence: [`deepseek/deepseek-r1`, `openai/o3-mini`]

The §8.5 JURY_TIERS entries define fallbacks **within their respective slot only**, not a shared cascade across all three slots. See §8.5 for the authoritative per-slot YAML definitions.
...
## §28.5 Relationship Between §28.2 REASONING_JUDGE_SLOTS and §8.5 JURY_TIERS

This section provides the canonical resolution for implementers who must reconcile the two definitions.

**Rule:** `§28.2` defines the **primary model** called first for each Reasoning jury slot. `§8.5` defines the **fallback sequence** invoked for that same slot when its primary model is unavailable or budget constraints trigger a downgrade. The two definitions are slot-scoped and complementary.

**Per-slot resolution table:**

| Slot | Primary (§28.2) | Fallback sequence (§8.5) | Invocation order |
|------|-----------------|--------------------------|------------------|
| R1 | `deepseek/deepseek-r1` | [`openai/o3-mini`, `qwen/qwq-32b`] | deepseek-r1 → o3-mini → qwq-32b |
| R2 | `openai/o3-mini` | [`deepseek/deepseek-r1`, `qwen/qwq-32b`] | o3-mini → deepseek-r1 → qwq-32b |
| R3 | `qwen/qwq-32b` | [`deepseek/deepseek-r1`, `openai/o3-mini`] | qwq-32b → deepseek-r1 → o3-mini |

**Implementation contract:** the `resolve_with_fallbacks` function in `src/llm/model_verifier.py` (see §28.3) MUST be called once per slot using that slot's own primary and its own fallback list from the table above. A single shared fallback list MUST NOT be used across all three Reasoning slots.

**§8.5 JURY_TIERS authoritative YAML** (defines per-slot fallback chains only; primary models are defined in §28.2):

```yaml
JURY_TIERS:
  R:
    R1:
      primary: "deepseek/deepseek-r1"       # canonical primary — see §28.2
      fallbacks:
        - "openai/o3-mini"
        - "qwen/qwq-32b"
    R2:
      primary: "openai/o3-mini"             # canonical primary — see §28.2
      fallbacks:
        - "deepseek/deepseek-r1"
        - "qwen/qwq-32b"
    R3:
      primary: "qwen/qwq-32b"              # canonical primary — see §28.2
      fallbacks:
        - "deepseek/deepseek-r1"
        - "openai/o3-mini"
```

**§33.6 LLM Routing — documentation reference note:**
All model pricing values shown in §33.6 documentation excerpts are for human-readable reference only and are reproduced from `src/llm/pricing.py`. `src/llm/pricing.py` is authoritative. Last verified: 2026-02-01. In particular: `google/gemini-2.5-pro` output price is **$10.00/M tokens** (not $5.00/M — earlier drafts contained that incorrect value). Any documentation excerpt showing a different figure for this model is outdated and must be corrected to match the canonical value in `src/llm/pricing.py`.
