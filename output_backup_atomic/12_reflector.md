...
|---|---|---|
| `SURGICAL` | ALL of: `len(items) ≤ 4`, all `affected_text` in distinct paragraphs (where paragraphs are delimited by `\n\n`; if no `\n\n` exists in `current_draft`, the entire draft is one paragraph and SURGICAL requires all items within that single paragraph), `iteration ≤ 2`, no `category == "structural_error"`, no scope-FULL item present | → Span Editor (§5.12) |
| `PARTIAL` | ANY of: `len(items) > 4`, items share a paragraph, `iteration > 2`, ≥1 `structural_error` present, but no FULL condition | → Writer (§5.7) |
| `FULL` | ANY of: dominant `category == "logical_contradiction"` spanning ≥3 paragraphs, ≥1 `severity == "CRITICAL"` with `category == "structural_error"`, Coherence Guard flagged HARD conflict in current section (§15.1) | → await_human directly, bypassing oscillation_check (§13.4) |
...
// PARTIAL — 6 items, iteration 2
{
  ...
  "items": [
    ...
    // + 5 more items across overlapping paragraphs
  ]
}
...
node. The aggregator node emits only `"fail_content"` on non-approval; it does **not**
perform surgical/partial/full routing — that distinction belongs exclusively here, where
`reflector_output` is guaranteed to be current.

FULL scope means the argument structure is fundamentally broken — oscillation detection
is irrelevant at this point. Route directly to human without oscillation evaluation.
SURGICAL and PARTIAL scopes proceed to `oscillation_check`, which then routes to
`span_editor` or `writer` respectively (or escalates to `await_human` on oscillation).
...
def route_after_reflector(state: DocumentState) -> str:
    ...
    if scope == "FULL":
        return "await_human"          # §13.4 — bypass oscillation_check entirely;
                                      # argument is structurally broken, oscillation
                                      # detection is irrelevant at this point
    return "oscillation_check"        # covers SURGICAL and PARTIAL;
                                      # oscillation_check routes to span_editor,
                                      # writer, or await_human (§13.4)
```

The `oscillation_check` node (§13) inspects `css_history` and draft embeddings, then
dispatches via `route_after_oscillation()`:

```python
def route_after_oscillation(state: DocumentState) -> str:
    if state["oscillation_detected"]:
        return "await_human"          # §13.4
    scope = state["reflector_output"]["dominant_scope"]
    if scope == "SURGICAL" and state["current_section"]["iteration"] <= 2:
