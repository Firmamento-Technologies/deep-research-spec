...
Dedicated agent for progressive context window management across multi-section documents. Invoked exactly once per section approval, after the section enters `approved_sections`, before the next section's Researcher call.
...
CONSTRAINTS:
  ...
  ALWAYS run after context_compressor node completes, never before writer
...
```
[context_compressor node] → [coherence_guard node] → [section_checkpoint node] → [route_next_section]
```

- **Trigger**: aggregator emits approval signal (CSS above threshold), routing to `context_compressor`.
...
The coherence guard runs **after** context compression and **before** section checkpoint so that conflicts detected by the coherence guard can block section approval. A section is only checkpointed (moved to `approved_sections` permanently) after the coherence guard confirms no hard conflicts.

LangGraph edge definition (see §4.5):

```python
g.add_edge("context_compressor", "coherence_guard")   # coherence check on compressed context
