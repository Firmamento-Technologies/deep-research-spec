Looking at the identified issue, I need to add a `route_after_writer()` function definition to §4.5. The function is referenced in the conditional edge `writer → route_after_writer → {mow_active: jury_multi_draft, standard: metrics_collector}` but never defined anywhere in the spec.

I'll add it alongside the other edge condition functions in §4.5.
...
EDGE CONDITION FUNCTIONS:
  ...
  def route_after_writer(state) -> Literal["mow_active", "standard"]:
      return "mow_active" if (
          state["config"]["user"]["quality_preset"] != "economy"
          and state["outline"][state["current_section_idx"]]["target_words"] >= 400
          and state["current_iteration"] == 1
          and not state.get("human_intervention_required")
      ) else "standard"

EDGE INTEGRATION WITH STANDARD GRAPH:

  writer → route_after_writer → {
      "mow_active": jury_multi_draft,
      "standard":   metrics_collector
  }
