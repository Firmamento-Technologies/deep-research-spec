Looking at the issue, I need to fix exactly one thing: in `§29.6 UserConfig`, the `max_budget_dollars` field already shows `le=500.0` in the current file content. Let me re-read carefully.

The issue states:
- §03_input_config.md DRSConfig sets `le=10_000.0` — this is in a **different file** (not this one)
- §01_vision.md MandatoryParams sets `le=500.0`
- §29.6 UserConfig sets `le=500.0` — already correct in the current file

The current file content already has `le=500.0` in `UserConfig`. The fix needed for **this file** is already correct. However, the preamble text at the top of the file (which appears to be a leftover editing artifact describing a *different* fix) needs to be removed, and the file should just be the clean spec.

Looking more carefully at the current file — the entire content starts with what appears to be editorial notes rather than the actual spec. The actual spec content starts at `# §29 — YAML Configuration & Pydantic Validation Schema`. The preamble is an artifact that should not be in the spec file.

I need to output the corrected file: remove the preamble editorial text, keep the spec as-is (since `le=500.0` is already correct in `UserConfig`).
...
models:
  ...
  jury_reasoning:                 # Cascading tier1→tier2→tier3 (see §8.5)
    tier1: "qwen/qwq-32b"
...
convergence:
  ...
  # Jury weights for CSS_final (must sum to 1.0); see §9.2
  jury_weights:
    reasoning: 0.35
    factual: 0.45
    style: 0.20
...
class ModelsConfig(BaseModel):
    ...
    jury_reasoning: JuryTiers
    jury_factual: JuryTiers
    jury_style: JuryTiers
