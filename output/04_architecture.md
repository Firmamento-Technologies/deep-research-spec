...
- `budget_estimator`: formula `cost = Σ sections × avg_iter × Σ(agent_tokens × price)`; blocks if projection > `max_budget_dollars × 0.80`; calls `populate_budget_thresholds()` (§9.3) to write resolved CSS thresholds into BudgetState
...
│  │                       │                                               │       │
│  │             [context_compressor]                                      │       │
│  │             (SKIPPED when current_section_idx == 0: no prior         │       │
│  │              context exists; no-op, returns unchanged state)          │       │
│  │                       │                                               │       │
│  │             [section_checkpoint]                                      │       │
│  │                       │                                               │       │
│  │            idx+1 < total? ─────┤                                     │       │
...
│  │                  oscillation? ──► [await_human]                      │       │
│  │                  SURGICAL + no osc ──► [span_editor] ──► [diff_merger]
│  │                  PARTIAL  + no osc ──► [writer]                      │       │
│  │              FULL ──► [await_human]                                  │       │
│  │                                                                       │       │
...
NODES: list[str] = [
    ...
    "writer",                                 # single node; MoW (asyncio.gather of
                                              # W-A/W-B/W-C + internal jury + Fusor)
                                              # is entirely internal to this node
                                              # when §7.1 conditions are met;
                                              # the graph sees only one 'writer' node
    ...
    "context_compressor",                     # runs AFTER coherence_guard; no-op on idx==0
    "section_checkpoint",
    "await_human",
    "budget_controller",                      # pass-through node; runs before Writer/Jury
    "post_qa",
...
def route_post_draft_gap(state: "DocumentState") -> Literal["gap", "no_gap"]:
    ...
    The iteration guard is enforced here at the graph level to provide a structural
    guarantee against infinite loops on iter >= 2.
...
def route_after_oscillation(state: "DocumentState") -> Literal[
    ...
    """
    Scope-aware routing after oscillation_check.
    If oscillation is detected, always escalate to human regardless of scope.
    If no oscillation, route based on reflector_output.dominant_scope:
      SURGICAL (and iteration <= 2) → span_editor
      PARTIAL (or SURGICAL iter > 2) → writer
    Budget threshold warnings are handled by the BudgetController node,
    which runs before every Writer/Jury call.
    """
    if state.get("oscillation_detected"):
        return "escalate_human"
    scope = state.get("reflector_output", {}).get("dominant_scope", "PARTIAL")
...
def route_after_context_compressor(state: "DocumentState") -> Literal[
    "section_checkpoint"
]:
    """
    Always routes to section_checkpoint. The context_compressor node itself
    implements the section_idx == 0 no-op guard internally (see §5.16 CONSTRAINTS:
    'NEVER invoke on section_idx == 0'). When section_idx == 0, context_compressor
    returns unchanged state (no-op) and the graph proceeds to section_checkpoint.
    This edge exists to make the no-op contract explicit.
    """
...
#   'oscillation_check' — SURGICAL or PARTIAL scope (oscillation_check then
#                          dispatches to span_editor, writer, or await_human)
#   'await_human'       — FULL scope (structural rewrite required; bypass oscillation)
...
def build_graph(checkpointer):
    ...
    # writer node: MoW (asyncio.gather of W-A/W-B/W-C + internal jury + Fusor)
    # is handled entirely internally within the writer node when §7.1 conditions
    # are met. The node returns a single draft regardless of internal path.
    # The graph sees only one 'writer' node with one outbound edge.
    g.add_edge("writer", "post_draft_analyzer")

    # Post-draft gap analysis — enforces iteration==1 guard at graph level
    g.add_conditional_edges("post_draft_analyzer", route_post_draft_gap,
        {"gap": "researcher_targeted", "no_gap": "style_linter"})

    # Targeted re-research: researcher_targeted sets targeted_research_active=True
    # in state; this flag is cleared by source_synthesizer before writer is invoked,
    # allowing writer to distinguish initial vs targeted research context.
    # researcher_targeted → citation_manager (shared) → citation_verifier →
    # source_sanitizer → source_synthesizer → writer (all shared edges reused)
    g.add_edge("researcher_targeted", "citation_manager")
    # citation_manager → citation_verifier already defined above
    ...
    # BudgetController as pass-through node before Jury invocations.
    ...
    # Span editor pipeline (SURGICAL scope from oscillation_check)
    ...
    # SURGICAL/PARTIAL → oscillation_check; FULL → await_human
    g.add_conditional_edges("reflector", route_after_reflector, {
        "oscillation_check": "oscillation_check",
        "await_human":       "await_human",
    })

    # oscillation_check: scope-aware dispatch.
    # route_after_oscillation reads reflector_output.dominant_scope AND
    # oscillation_detected to determine the correct next node.
    #   oscillation_detected=True  → await_human (any scope)
    #   SURGICAL + iter<=2         → span_editor
    #   PARTIAL (or SURGICAL>2)    → writer
    ...
    #      HARD conflict → await_human; no compression or checkpoint occurs.
    #   2. context_compressor runs AFTER coherence_guard confirms no HARD conflict.
    #      When current_section_idx == 0: context_compressor is a no-op (§5.16
    #      CONSTRAINTS: 'NEVER invoke on section_idx == 0'). The node returns
    #      unchanged state; the graph edge still proceeds to section_checkpoint.
    ...
    # context_compressor always routes to section_checkpoint (no-op on idx==0)
...
class BudgetState(TypedDict):
    ...
    # Values are read from §9.3 THRESHOLD_TABLE. MUST NOT be hardcoded.
...
class AggregatorVerdict(TypedDict):
    ...
    css_composite: float         # weighted average for Run Report
...
class ReflectorOutput(TypedDict):
    ...
    dominant_scope: Literal["SURGICAL", "PARTIAL", "FULL"]  # alias; same field
...
class DocumentState(TypedDict):
    ...
    css_history: list[float]       # css_content per iteration this section
    draft_embeddings: list[list[float]]  # for semantic oscillation check §13.2

    # ── Aggregator CSS outputs (written by Aggregator node — §9.7) ────────────
    css_content_current: float     # current section, current iteration CSS_content
    css_style_current: float       # current section, current iteration CSS_style
    css_composite_current: float   # Run Report only; not used for routing
    ...
    targeted_research_active: bool # set True by researcher_targeted node;
                                   # cleared by source_synthesizer before writer call;
                                   # allows writer and downstream agents to distinguish
                                   # initial research path from targeted re-research path
...
**Cross-refs:** agents §5, jury §8, aggregator §9, budget §19, oscillation §13, coherence §15, MoW §7, panel §11, reflector §12, style linter/fixer §5.9–5.10, context compressor §5.16, section_checkpoint §5.21, length_adjuster §5.22.
