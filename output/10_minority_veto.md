...
L1 veto detection is implemented inside `route_after_aggregator()` in **§9.4** (canonical
implementation). The pseudocode below illustrates the detection logic consumed by §9.4:

```python
# Pseudocode — detection logic consumed by route_after_aggregator() in §9.4
...
L2 veto detection is implemented inside `route_after_aggregator()` in **§9.4** (canonical
implementation). The helper functions below define the detection logic:
...
> **NOTE — route_after_aggregator() canonical implementation**: The single canonical implementation of `route_after_aggregator()` is defined in **§9.4** of `09_css_aggregator.md`. That implementation handles both L1 and L2 veto detection, budget hard stop, missing evidence, CSS-based routing, and force_approve — in the correct priority order. No other file defines or overrides this function. The L1 and L2 veto logic described in §10.1 and §10.2 defines the detection semantics and system behavior; the routing function itself lives exclusively in §9.4.
>
> The `route_after_aggregator()` return literals are exclusively `AggregatorRoute` values:
> `"approved"`, `"force_approve"`, `"veto"`, `"panel"`, `"fail_missing_ev"`,
> `"fail_style"`, `"fail_reflector"`, `"budget_hard_stop"`.
> Scope values such as `"surgical"`, `"partial"`, and `"full"` are **not** aggregator-level routes —
> they are determined by `route_after_reflector()` (§12.5) after the reflector runs.
...
| Aggregator routing post-veto (canonical implementation) | §9.4 |
